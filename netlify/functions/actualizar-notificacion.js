...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { idAviso, titulo, mensaje, tipo = 'informacion', destinatarios = 'todos', usuariosSeleccionados = [] } = data;

    // Validar que se proporcionen los datos requeridos
    if (!idAviso || !titulo || !mensaje) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ 
          success: false, 
          message: 'El ID del aviso, título y el mensaje son requeridos' 
        }),
      };
    }
    ...
    try {
      ...
      const [avisos] = await connection.execute(
        'SELECT idAviso, idAutor, idComunidad FROM Aviso WHERE idAviso = ?',
        [idAviso]
      );
      ...
      if (aviso.idAutor !== decodedToken.id) {
        ...
        return {
          statusCode: 403,
          headers,
          body: JSON.stringify({ success: false, message: 'No tiene permiso para editar esta notificación' }),
        };
      }
      ...
      if (lecturas[0].total > 0) {
        ...
        return {
          ...
          body: JSON.stringify({ 
            success: false, 
            message: 'No se puede editar una notificación que ya ha sido leída por los usuarios' 
          }),
        };
      }
      ...
      try {
        ...
        await connection.execute(
          'UPDATE Aviso SET titulo = ?, contenido = ?, tipo = ?, destinatarios = ? WHERE idAviso = ?',
          [titulo, mensaje, tipo, destinatarios, idAviso]
        );

        // Eliminar los registros existentes en UsuarioAviso para este aviso
        await connection.execute(
          'DELETE FROM UsuarioAviso WHERE idAviso = ?',
          [idAviso]
        );
        ...
        // Obtener los detalles del aviso actualizado para devolverlos
        const [avisosActualizados] = await connection.execute(
        ...
        if (destinatarios === 'seleccionados') {
          ...
          destinatariosInfo = usuariosAsignados;
        }
