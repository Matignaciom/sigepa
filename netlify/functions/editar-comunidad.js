  ...
  try {
    ...
    if (decodedToken.role !== 'Administrador') {
      ...
      return {
        ...
        body: JSON.stringify({ 
          success: false, 
          message: 'Acceso denegado. Solo los administradores pueden editar información de la comunidad.' 
        }),
      };
    }

    const userId = decodedToken.id;
    console.log(`Administrador ID ${userId} está editando información de comunidad`);
    ...
    if (email_contacto) {
      ...
      if (!emailRegex.test(email_contacto)) {
        return {
          ...
          body: JSON.stringify({ 
            success: false, 
            message: 'Formato de correo electrónico de contacto inválido' 
          }),
        };
      }
    }
    ...
    try {
      ...
      console.log(`Editando comunidad ID: ${comunidadId}`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`Comunidad ID ${comunidadId} actualizada correctamente`);
    } catch (error) {
      ...
      return {
        ...
        body: JSON.stringify({ 
          success: false, 
          message: 'Error al actualizar información de la comunidad',
          error: error.message 
        }),
      };
    }

    // Obtener la información actualizada de la comunidad
    let comunidadData;
    try {
      const query = `
        SELECT 
      ...
      if (comunidades.length === 0) {
        throw new Error('Comunidad no encontrada después de actualización');
      }
      
      comunidadData = comunidades[0];
    } catch (error) {
      console.error('Error al obtener datos actualizados de la comunidad:', error);
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        message: 'Información de la comunidad actualizada exitosamente',
        data: comunidadData
      }),
    };
  } catch (error) {
    console.error('Error general al editar comunidad:', error);
