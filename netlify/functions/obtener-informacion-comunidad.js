  ...
  try {
    ...
    if (decodedToken.role !== 'Administrador') {
      ...
      return {
        ...
        body: JSON.stringify({ 
          success: false, 
          message: 'Acceso denegado. Solo los administradores pueden ver información detallada de la comunidad.' 
        }),
      };
    }

    const userId = decodedToken.id;
    console.log(`Administrador ID ${userId} está consultando información de comunidad`);
    ...
    try {
      ...
      console.log(`Consultando comunidad ID: ${comunidadId}`);
    } catch (error) {
    ...
    // Obtener información detallada de la comunidad
    let comunidadData;
    try {
      const query = `
        SELECT 
          ...
          (SELECT COUNT(*) FROM GastoComun WHERE idComunidad = c.idComunidad AND fechaVencimiento >= CURDATE()) as gastos_pendientes,
          (SELECT COUNT(*) FROM GastoComun WHERE idComunidad = c.idComunidad AND fechaVencimiento < CURDATE()) as gastos_vencidos
        FROM Comunidad c
        WHERE c.idComunidad = ?
      `;
      ...
      // Obtener estadísticas adicionales de pagos para la comunidad
      const estadisticasQuery = `
        SELECT 
          (SELECT SUM(montoPagado) FROM Pago p 
           JOIN Parcela par ON p.idParcela = par.idParcela 
           WHERE par.idComunidad = ? AND YEAR(p.fechaPago) = YEAR(CURDATE())) as monto_total_anual,
          (SELECT COUNT(*) FROM GastoParcela gp 
           JOIN Parcela par ON gp.idParcela = par.idParcela 
           WHERE par.idComunidad = ? AND gp.estado = 'Atrasado') as pagos_atrasados,
          (SELECT COUNT(*) FROM GastoParcela gp 
           JOIN Parcela par ON gp.idParcela = par.idParcela 
           WHERE par.idComunidad = ? AND gp.estado = 'Pagado') as pagos_realizados
      `;
      
      const [estadisticas] = await connection.execute(estadisticasQuery, [comunidadId, comunidadId, comunidadId]);
      
      if (estadisticas.length > 0) {
        comunidadData.estadisticas = {
          monto_total_anual: estadisticas[0].monto_total_anual || 0,
          pagos_atrasados: estadisticas[0].pagos_atrasados || 0,
          pagos_realizados: estadisticas[0].pagos_realizados || 0
        };
      }
      
      // Formatear fechas para mejor legibilidad
      if (comunidadData.fecha_creacion) {
        comunidadData.fecha_creacion = new Date(comunidadData.fecha_creacion).toISOString().split('T')[0];
      }
      ...
      return {
        ...
        body: JSON.stringify({ 
          success: false, 
          message: 'Error al obtener información detallada de la comunidad',
          error: error.message 
        }),
      };
    }
    ...
    console.error('Error general al obtener información de la comunidad:', error);
