  ...
  try {
    ...
    if (decodedToken.role !== 'Administrador') {
      ...
      return {
        ...
        body: JSON.stringify({ 
          success: false, 
          message: 'Acceso denegado. Solo los administradores pueden editar su perfil usando esta función.' 
        }),
      };
    }

    const userId = decodedToken.id;
    console.log(`Editando perfil para el administrador ID: ${userId}`);
    ...
    try {
      ...
      console.log(`Perfil actualizado para el administrador ID: ${userId}`);
    } catch (error) {
    ...
    try {
      ...
      // Obtener estadísticas adicionales para el administrador
      const statsQuery = `
        SELECT 
      ...
      const [stats] = await connection.execute(statsQuery, [userData.idComunidad, userData.idComunidad]);
      
      if (stats.length > 0) {
        userData.stats = stats[0];
      }
    } catch (error) {
    ...
    console.error('Error general al editar perfil de administrador:', error);
